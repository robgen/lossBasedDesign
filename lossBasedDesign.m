classdef lossBasedDesign
    ...
    methods % macro-methods
        ...
        function self = lossBasedDesign(structureType, options)
            
            if nargin < 1, structureType = 'Frame'; end
        ...
        function self = getSeedEAL(self, GPfullFit)
            ...
            self = getVulnStructure(self, GPfullFit);
            if self.parameters.NSC.explicitNSC
                self.vulnDisaggregated(1,1,...
                    size(self.vulnDisaggregated,3) + ...
                    2*self.parameters.(self.structureType).Nstoreys + ...
                    1) = 0; % pre-allocation
                self = getVulnNSCD(self, GPfullFit.PSDMformula);
                self = getVulnNSCA(self);
            else
                self.vulnDisaggregated(1,1,...
                    size(self.vulnDisaggregated,3) + 1) = 0; % pre-allocation
            end
            self = getAggregateVulnerability(self);
            self = getEAL(self);
        ...
        function self = getDesignCandidates(self, EALtarget)
            ...
            for n = 1 : numel(self.isEALTARGET)
                if self.isEALTARGET(n)
                    ...
                    for ds = 1 : size(self.ductThresholds,2)
                        self.CDRs(n,ds) = CSMnbs(...
                            self.parameters.Hazard.codeSpectra{ds}, ...
                            self.pushCandidates{k}, 1/9.81, deltaY, 'NOplot', ...
                            Cevd.(self.hyst{n}), 0.5, 0.0001, ...
                            self.ductThresholds(n,ds)*deltaY, 'y', 0, 0, 0, 'n');
                        
                        self.CDRcandidates(k,ds) = self.CDRs(n,ds);
                    end
                end
            end
        ...
        function self = selectDesignSDoF(self, selectedSDoF, comparativePush)
            ...
            figure('Position', [832   256   560   420]); hold on;
            for ds = size(self.ductThresholds,2) : -1 : 1
                p(ds) = plot(self.parameters.Hazard.codeSpectra{ds}(:,3), ...
                    self.parameters.Hazard.codeSpectra{ds}(:,2), ...
                    'LineWidth', 2, 'Color', colDS(ds,:), ...
            ...
            for k = numel(self.pushCandidates) : -1 : 1
                ...
                candPlot(k) = plot(self.pushCandidates{k}(:,1), ...
                    self.pushCandidates{k}(:,2), lineType, ...
                    'Color', 0.5*[1 1 1], 'DisplayName', 'Design candidates');
            end
            ...
            slMin = 1; slMax = numel(self.pushCandidates);
            self.slider = uicontrol('Style', 'slider', ...
                'Position',[309,392,200,20],  ...
                'Min', slMin, 'Max', slMax, 'SliderStep', [1 1]./(slMax-slMin), ...
            ...
            function setSlider(src, event) %#ok<INUSD>
                ...
                candPlot(src.Value).Color = [0 0 0];
                candPlot(src.Value).LineWidth = 2;
                title(src.Value)
            end
        ...
        function self = getMemberDetailing(self)
        ...
        function plotEAL(self)
            ...
            p(4) = scatter3(self.FY(self.indCANDIDATE==self.isDesign), ...
                self.MU(self.indCANDIDATE==self.isDesign), ...
                self.EAL(self.indCANDIDATE==self.isDesign), 1000, ...
                'p', 'MarkerFaceColor', [0.93,0.69,0.13], ...
                'MarkerEdgeColor', 'k', 'DisplayName', 'Design SDoF');
            view(gca,[81.3000001847013 19.9181347341946]);
            legend1 = legend(p);
            set(legend1, 'Position',[0.657691964285714 0.785714285714286 0.250892857142857 0.101190476190476] )
            xlabel('f_y [-]')
            ylabel('\mu_{cap} [-]')
            zlabel('EAL [%]')
            set(gca, 'FontSize', 18)
        ...
        function plotMAFEds(self)
            
            colDS = [ 0.392   0.831   0.074
                        1.00    1.00    0.07
                        0.929   0.694   0.125
                        0.635   0.078   0.184];
        
            figure('Position', [271   256   560   420]);
            for ds = 1 : size(self.mafeDS,2)
                surf(self.FY, self.MU, ...
                    reshape(self.mafeDS(:,ds), size(self.FY,1), size(self.FY,2)), ...
                    'FaceAlpha', 0.8, 'FaceColor',colDS(ds,:), ...
                    'DisplayName', 'EAL seed SDoFs'); hold on
            end
            
            for ds = size(self.mafeDS,2) : -1 : 1
                scatter3(self.FY(self.indCANDIDATE==self.isDesign), ...
                    self.MU(self.indCANDIDATE==self.isDesign), ...
                    self.mafeDS(self.indCANDIDATE==self.isDesign,ds), 1000, ...
                    'p', 'MarkerFaceColor', [0.93,0.69,0.13], ...
                    'MarkerEdgeColor', 'k', 'DisplayName', 'Design SDoF');
                
                labels{ds} = sprintf('DS%d %1.2e', ds, ...
                    self.mafeDS(self.indCANDIDATE==self.isDesign,ds));
            end
            set(gca, 'Zscale', 'log')
            view(gca,[108.841308121894 21.7774611928815]);
            legend1 = legend(labels);
            set(legend1, 'Position', [0.157 0.222 0.252 0.194] )
            xlabel('f_y [-]')
            ylabel('\mu_{cap} [-]')
            zlabel('MAFE(DS) [-]')
            set(gca, 'FontSize', 18)
        ...
        function plotMemberDetailing(self, comparativePush, comparativeOther)
            
            if nargin < 2; comparativePush = 0; end
            if nargin < 3; comparativeOther = 0; end
            
            % General part
            effMass = self.parameters.(self.structureType).effMass;
            ...
            % LRS-specific part
            self.(['plot' self.structureType 'MemberDetailing'])(comparativeOther)
        end
    ...
    methods % micro methods
        ...
        function self = getVulnStructure(self, GPfullFit)
            ...
            if isnan(self.parameters.FragVuln.fixedBeta)
                self.fragStDev = ( (dummyStDev(:,end) * ...
                    ones(1,size(self.fragMedian,2))).^2 + ...
            ...
            for n = numel(self.fy): -1 : 1
                for ds = size(self.fragMedian,2) : -1 : 1
                    fragilities(:,ds,n) = logncdf(self.IMdef,...
                        log(self.fragMedian(n,ds)), self.fragStDev(n,ds));
                end
            end

            if self.isDLRglobal
                self = getVulnStructureGlobal(self, fragilities);
            else
                % if the displacement shape does not change as a function
                % of the SDoF equivalent displacement, and the DS thresholds
                % are a linear function of the ductility capacity of
                % the seed SDoF and the displacement shape, the fragility of
                % each storey is equivalent to the global fragility
                self = getVulnStructureStorey(self, fragilities);
            end

        end


        function self = getVulnStructureGlobal(self, fragilities)

            for n = numel(self.fy): -1 : 1
                vulnerability = ...
                    VULNERABILITYbuilding([self.IMdef, fragilities(:,:,n)], ...
                    self.costRatioStructure * ...
                    self.parameters.FragVuln.damageToLoss, 'NOplot', ...
                    'S_a(T_1)', 0, self.parameters.FragVuln.fxIndLosses, ...
                    self.parameters.FragVuln.parIndLosses); % ind losses should not be considered here
                self.vulnDisaggregated(:,n) = vulnerability(:,2);
            end
        end


        function self = getVulnStructureStorey(self, fragilities)
            % refactor this to remove the "global" one

            for n = numel(self.fy): -1 : 1
                for s = self.parameters.(self.structureType).Nstoreys : -1 : 1
                    vulnStorey = VULNERABILITYbuilding(...
                        [self.IMdef, fragilities(:,:,n)], ...
                        self.costRatioStructure * ...
                        self.parameters.FragVuln.damageToLoss(s,:), ...
                        'NOplot', 'S_a(T_1)', 0); % only direct
                    self.vulnDisaggregated(:,n,s) = vulnStorey(:,2);
                end
            end
        end


        function self = getVulnNSCD(self, PSDMformula)
            
            duct = zeros(numel(self.IMdef),1);
            for n = 1 : numel(self.fy)
                R = self.IMdef / self.fy(n);

                duct(R<=1,:) = R(R<=1);
                duct(R>1,:) = PSDMformula.mu(R(R>1), self.powerLaw(n,1), 0); % mean
                deltaSDoF = duct * self.parameters.(self.structureType).deltaYield;
                driftGivenIM = ...
                    self.parameters.(self.structureType).driftGivenIM(self, deltaSDoF);
                for s = 1 : self.parameters.(self.structureType).Nstoreys                    
                    self.vulnDisaggregated(:,n,1+s) = ...
                        self.parameters.NSC.Dsens(s).SLF(driftGivenIM(:,s));
                end
            end
        end


        function self = getVulnNSCA(self)
            
            SfactorP58 = ones(numel(self.IMdef),1);
            SAoverPGAmean = mean( self.parameters.Hazard.intensityHazard ./ ...
                self.parameters.Hazard.intensityHazard(:,1) );

            Nstoreys = self.parameters.(self.structureType).Nstoreys;
            for n = 1 : numel(self.fy)
                R = self.IMdef / self.fy(n);

                [~, closestT] = min(abs(self.parameters.Hazard.periodsHazard - self.t(n)));
                pgaGivenIM = self.IMdef ./ SAoverPGAmean(closestT);
                SfactorP58(R>1) = R(R>1);
                pfaGivenIM = self.parameters.(self.structureType).accShape(...
                    self.t(n), SfactorP58) .* pgaGivenIM;
                for s = 1 : Nstoreys                    
                    self.vulnDisaggregated(:,n,1+Nstoreys+s) = ...
                        self.parameters.NSC.Asens(s).SLF(pfaGivenIM(:,s));
                end
            end
        end


        function self = getAggregateVulnerability(self)
            % aggregate and add indirect losses
            
            vulnDirect = sum(self.vulnDisaggregated, 3);
            self.vulnDisaggregated(:,:,end) = ...
                self.parameters.FragVuln.fxIndLosses( ...
                vulnDirect, self.parameters.FragVuln.parIndLosses);
            self.vulnerabilities = sum(self.vulnDisaggregated, 3);
        end
        ...
        function self = buildStoreyLossFx(self)
            ...
            groups = {'Dsens', 'Asens'};
            for g = 1 : 2
                for n = self.parameters.(self.structureType).Nstoreys : -1 : 1
                    p = self.parameters.NSC.(groups{g})(n).SLFpars;
                    switch self.parameters.NSC.(groups{g})(n).SLFshape
                        case 'Papadopoulos'
                            self.parameters.NSC.(groups{g})(n).SLF = @(edp) ...
                                self.parameters.NSC.(groups{g})(n).costRatio * ...
                                ( p(5) * (edp.^p(1) ./ (p(2)^p(1) + edp.^p(1))) + ...
                                (1-p(5)) * (edp.^p(3) ./ (p(4)^p(3) + edp.^p(3))) );
                        case 'Weibull'
                            warning('Code Weibull SLF')
                            self.parameters.NSC.(groups{g})(n).SLF = @(edp) 0;
                    end
                end
            end
        end
        ...
        function self = calculateAccShape(self)
            ...
            hRatio = self.parameters.(self.structureType).heightStorey' / ...
                self.parameters.(self.structureType).heightStorey(end);

            self.parameters.(self.structureType).accShape = ...
    ...
    methods(Access = 'private')
        ...
        function self = setAllParameters(self, options)
            ...
            macroFieldsPar = {'SDoF', 'FragVuln', 'NSC', 'Hazard', 'Frame', 'Wall'};
            ...
            microFieldsPar{3} = {'explicitNSC', 'Dsens', 'Asens'};
            microFieldsParVals{3} = {false, struct, struct};

            microFieldsPar{4} = {'faultRate', 'periodsHazard', 'extrapMode', 'MAFEhazard', 'intensityHazard', 'codeSpectra' };
            microFieldsParVals{4} = { 0.58, [0,0.1,0.15,0.2,0.3,0.4,0.5,0.75,1,1.5,2], 'powerLaw', ...
            ...
            microFieldsPar{5} = {'fc' 'fct' 'epsYsteel', 'lengthBeam', 'heightBeam', ...
                'heightColumn', 'Nparallel', 'massesBuilding', 'Nstoreys', 'Nbays', ...
                'heightInterstorey', 'gammaMomentCols', 'limitBeams', 'DDBDdeltaYcorrector'};
                microFieldsParVals{5} = {25, 1.2, 300/200000, 6, 0.55, 0.6, 4, 300, ...
                4, 3, 3.6, 0.6, 4, 1};

            microFieldsPar{6} = {'fc' 'fySteel', 'barAvgDiam', 'lengthWall', 'Nparallel', ...
                'massesBuilding', 'Nstoreys', 'heightInterstorey'};
                microFieldsParVals{6} = {25, 300, 20, 4, 2, 300, 4, 3.6};
        ...
        function self = setLossAssessmentMode(self)
            ...
            if size(self.parameters.FragVuln.damageToLoss,1) == 1
                self.isDLRglobal = true;

                self.labelsVuln = {'Structure'};
            else
                self.isDLRglobal = false;

                self.labelsVuln = strcat('Structure', ...
                    storeyLabels(1:self.parameters.(self.structureType).Nstoreys));
            end
            
            if self.parameters.NSC.explicitNSC
                self = buildStoreyLossFx(self);
                self.costRatioStructure = 1 - ...
                    sum([self.parameters.NSC.Dsens(:).costRatio]) - ...
                    sum([self.parameters.NSC.Asens(:).costRatio]);

                self.labelsVuln(end+1: ...
                    end+self.parameters.(self.structureType).Nstoreys) = ...
                    strcat('NSCD', ...
                    storeyLabels(1:self.parameters.(self.structureType).Nstoreys));
                self.labelsVuln(end+1: ...
                    end+self.parameters.(self.structureType).Nstoreys) = ...
        ...
        function self = setDSthresholds(self)
            ...
            for ds = numel(self.parameters.FragVuln.ductDS) : -1 : 1
                if self.parameters.FragVuln.ductDSmultiplyDS4(ds) == 0
                    multiplierMuDS4 = ones(numel(self.mu),1);
                else
                    multiplierMuDS4 = self.mu;
                end
                self.ductThresholds(:,ds) = ...
                    self.parameters.FragVuln.ductDS(ds) .* multiplierMuDS4;
            end
        ...
        function self = collectDesignProperties(self, selectedSDoF)
            ...
            self.propDesign.dispThresholds = ...
                self.ductThresholds(n,:) * ...
                self.parameters.(self.structureType).deltaYield;
            
            % this stuff depends on the structure type. It should not be here
            % Also, it's useless
            %             dEff = sum(...
            %                 self.parameters.Frame.massesBuilding .* ...
            %                 self.parameters.Frame.dispShape .^ 2) / ...
            %                 sum(self.parameters.Frame.massesBuilding .* ...
    ...
    methods(Static)
        ...
        function allOut = allCalculationsOneCase(GPfullFit, ...
                capCurveBil, hysteresis, dispThresholds, DLR, hazard, ...
                fixedBeta, fxIndLosses, parIndLosses)
            % NOTE: this fx violates the single responsability principle
            
            if nargin < 7; fixedBeta = NaN; end
            if nargin < 8
                fxIndLosses = @(Ldir,par)par(1)*normcdf(Ldir,0.5,par(2));
            end
            if nargin < 9; parIndLosses = [0, 0.15]; end
            
            allOut.deltaY = capCurveBil(2,1);
            allOut.deltaU = capCurveBil(3,1);
            allOut.fY = capCurveBil(2,2);
            allOut.fU = capCurveBil(3,2);
            allOut.T = 2*pi * (allOut.deltaY ./ (allOut.fY*9.81)).^0.5;
            allOut.hard = (allOut.fU-allOut.fY)/(allOut.deltaU-allOut.deltaY) / (allOut.fY/allOut.deltaY);
            ductThresholds = dispThresholds ./ allOut.deltaY;
            
            [allOut.fragMedian, dummyStDev, allOut.powerLaw, allOut.isExtrapolated] = ...
                GPfullFit.predictFragGP(...
                hysteresis, allOut.T, allOut.fY, allOut.hard, ductThresholds);
            
            if isnan(fixedBeta)
                allOut.fragStDev = ones(size(dummyStDev)) * dummyStDev(end);
            else
                allOut.fragStDev = ones(size(dummyStDev)) * fixedBeta;
            end
            
            if any(allOut.isExtrapolated==1)
                warning('Extrapolation: some Seed SDoF exceeds the limits of the GP')
            end
            
            IMdef = linspace(0, 2.5, 100)';            
            for ds = size(allOut.fragMedian,2) : -1 : 1
                allOut.fragilities(:,ds) = logncdf(IMdef,...
                    log(allOut.fragMedian(ds)), allOut.fragStDev(ds));
            end
            allOut.vulnerability = VULNERABILITYbuilding(...
                [IMdef, allOut.fragilities], DLR, 'NOplot', ...
                'S_a(T_1)', 0, fxIndLosses, parIndLosses);
            
            for l = size(hazard.intensityHazard,1) : -1 : 1
                intHaz(l) = interp1(hazard.periodsHazard, ...
                    hazard.intensityHazard(l,:), allOut.T);
            end
            
            x1 = log(0.01);
            x2 = log(intHaz(1));
            y1 = log(hazard.faultRate);
            y2 = log(hazard.MAFEhazard(1));
            
            k1 = (y2-y1)/(x2-x1);
            k2 = exp( -(y2-y1)/(x2-x1) * x1 + y1 );
            
            % add the points to the hazard curve
            imExtrap = 0.01 : 0.01 : intHaz(1);
            mafeExtrap = k2*imExtrap.^k1;
            
            allOut.hazCurve(:,1) = ...
                [0; imExtrap'; intHaz'];
            allOut.hazCurve(:,2) = ...
                [mafeExtrap(1); mafeExtrap'; hazard.MAFEhazard(:)];
            
            allOut.EAL = EALcalculator(...
                allOut.vulnerability, allOut.hazCurve, 'NOplot') * 100;
        end
        
        
        function plotAllCalculationsOneCase(allOut)
            
            figure; hold on
            plot(allOut.vulnerability(:,1), allOut.vulnerability(:,2), ...
                'LineWidth', 2)
            plot(allOut.vulnerability(:,1), allOut.vulnerability(:,3), ...
                'LineWidth', 2)
            set(gca, 'FontSize', 18)
            xlabel('S_a(T_1) [g]')
            ylabel('Loss Ratio [-]')
            ...
            yyaxis right
            set(gca, 'YColor', 'k')
            plot(allOut.hazCurve(:,1), allOut.hazCurve(:,2), ...
                'LineWidth', 2)
            ylabel('MAFE(S_a(T_1)) [-]')
        end
    ...
    methods % micro methods - Frame
        ...
        function self = FrameDBDgeneral(self, manualDeltaYield)
            ...
            frame.effDispGivenUnitRoofDisp = ...
                sum(frame.massesBuilding .* (frame.dispShape.^2)) / ...
            ...
            if manualDeltaYield == 0
                ...
                frame.deltaYield = self.parameters.Frame.DDBDdeltaYcorrector * ...
            ...
            frame.driftGivenIM = @(self, deltaSDoF) ...
                100 * deltaSDoF .* self.parameters.Frame.driftShape' ./ ...
                self.parameters.Frame.effDispGivenUnitRoofDisp;
    ...
    methods % micro methods - Wall
        ...
        function self = WallDBDgeneral(self, manualDeltaYield)
            ...
            totDispUnitPlasticCurv = ...
                wall.dispProfileYield + wall.dispUnitPlasticCurv;
            wall.driftUnitPlasticCurv = ...
                (totDispUnitPlasticCurv - [0; totDispUnitPlasticCurv(1:end-1)]) ./ ...
                wall.heightInterstorey;
            
            wall.driftGivenIM = @(self, deltaSDoF) ...
                100 * deltaSDoF .* self.parameters.Wall.driftUnitPlasticCurv' ./ ...
                self.parameters.Wall.dEffUnitPlasticCurv;
